...
export const getAllFeatureCategories = async (
    req: Request<unknown, unknown, unknown, queryFindFeatureCategoryData>,
    res: Response
): Promise<Response> => {
...
export const storeOrUpdateFeatureCategory = async (
    ...
    try {
        ...
        return res.status(200).json({
            success: true,
            data: typeof storeOrUpdateFeatureCategory === 'string' ? {} : storeOrUpdateFeatureCategory,
            message:
                typeof storeOrUpdateFeatureCategory === 'string'
                    ? storeOrUpdateFeatureCategory
                    : `Feature category data successfully ${status}`
        });
    } catch (err) {
        next(err);
    }
...
export const deleteFeatureCategory = async (
    req: Request,
    res: Response,
    next: NextFunction
): Promise<Response | undefined> => {
